...
def callback(
    ...
    accounts_collection.update_one(
        ...
        {
            "$inc": {"balance": -transfer_amount}, # Subtract transfer amount from sender's balance
            "$push": {"transfers_complete": transfer_id}, # Add transfer ID to transder history of sender account
        },
        session=session,
    )
    ...
    accounts_collection.update_one(
        ...
        {
            "$inc": {"balance": transfer_amount}, # Add transfer amount to receiver's balance
            "$push": {"transfers_complete": transfer_id}, # Add transfer ID to transder history of receiver account
        },
        session=session,
    )
